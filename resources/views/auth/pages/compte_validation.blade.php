<!DOCTYPE html>
<html lang="it">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Email LBE Finanza</title>

    <!-- Favicon -->
    <link rel="shortcut icon" href="assets/images/logo/favicon.png">

    <!-- page css -->

    <!-- Core css -->
    <link href="assets/css/app.min.css" rel="stylesheet">
    <script src="https://cdn.tailwindcss.com"></script>

</head>

<body>
    <div class="app">
        <div class="container-fluid">
            <div class="d-flex full-height p-v-20 flex-column ">
                <div class=" d-md-flex p-h-40">
                    <img src="site_web/img/logo.png" alt="">
                </div>
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <div class="p-v-30">
                              
                                <div class="flex ">
                                    <h2 class="font-bold text-2xl">Conto in fase di convalida</h2>
                                    <img src="assets/images/logo/envelope-solid.svg" class="w-8 h-8 ml-2" alt="">
                                </div>

                                <p>Il tuo account è in fase di convalida. Ti preghiamo di attendere mentre esaminiamo le informazioni fornite. Una volta completata la convalida, puoi accedere</p>
                                <div class="flex flex-row flex-wrap">
                               
                                <a href="/" class="btn btn-primary btn-tone mt-2">Indietro </a>
                               </div>
                            </div>
                        </div>
                        <div class="col-md-6 m-l-auto d-none d-md-block">
                            <img class="img-fluid" src="assets/images/others/error-1.png" alt="">
                        </div>
                    </div>
                </div>
                
            </div>
        </div>
    </div>

    
    <!-- Core Vendors JS -->
    <script src="assets/js/vendors.min.js"></script>

    <!-- page js -->

    <!-- Core JS -->
    <script src="assets/js/app.min.js"></script>

</body>

</html>